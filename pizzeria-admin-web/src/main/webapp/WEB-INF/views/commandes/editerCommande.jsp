...
<body class="container">
	...
	<h1>
		<c:out
			value="${ editPage ? 'Editer la commande' : 'Créer une commande' }" />
	</h1>
	...
	<form method="POST">
		...
		<div class="form-group">
			<label for="nom">Numero</label> <input type="text"
				class="form-control" id="nom" name="numero"
				value="${commande.numeroCommande}" placeholder="Numero de la commande (chaine de caractères)"
		...
		<div class="form-group">
			<label for="statut">Statut</label> <select name="statut" id="statut"
				...
				<c:forEach var="statut" items="${statuts}">
					<option value="${statut}"
						<c:if test="${commande.statut == statut}">selected</c:if>>${statut.libelle}</option>
				</c:forEach>
			</select>
		</div>
		...
		<div class="form-group">
			<label for="client">Client</label> <select name="client" id="client"
				class="form-control">
						...
						<div class="row">
							...
							<div class="col-md-1">
								<input type="number" class="form-control"
									name="${pizzaCmd.pizza.code}" value="${pizzaCmd.quantite}"
									min="0">
							</div>
						</div>
					</c:forEach>
				...
				<c:otherwise>
					<c:forEach var="pizza" items="${pizzas}">
						<div class="row">
							...
							<div class="col-md-1">
								<input type="number" class="form-control" name="${pizza.code}"
									value="0" min="0">
							</div>
						</div>
					</c:forEach>
